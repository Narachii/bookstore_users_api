...
const (
	...
	queryByStatus               = "SELECT id, first_name, last_name, email, date_created, status FROM users WHERE status=?"
...
func (user *User) FindByStatus(status string) ([]User, *errors.RestErr) {
	stmt, err := users_db.Client.Prepare(queryByStatus)
	if err != nil {
		logger.Error("error when trying to find users by statement", err)
...
func (user *User) FindByEmailAndPassword() *errors.RestErr {
	...
	if getErr := result.Scan(&user.Id, &user.FirstName, &user.LastName, &user.Email, &user.DateCreated, &user.Status); getErr != nil {
		...
		logger.Error("error when tying to get user by email and password", getErr)
		return errors.NewInternalServerError("database error")
	}
	return nil
}
